...
使用方法:
    python check_variable_reassignment_v13.py src/
    python check_variable_reassignment_v13.py src/test/test_file.erl
"""
...
class VariableAssignmentChecker:
    ...
    def extract_variables_from_pattern(self, pattern: str) -> Set[str]:
        """从模式中提取变量名，过滤匿名变量和字符字面量"""
        variables = set()
        # 移除字符字面量（$X形式）
        pattern_no_char_literals = re.sub(r'\$[A-Za-z0-9_]', '', pattern)
        
        for var in re.findall(r'\b([A-Z_][a-zA-Z0-9_]*)\b', pattern_no_char_literals):
            # 过滤匿名变量 _ 和下划线开头的变量
    ...
    def get_assigned_variables(self, line: str, prev_line: Optional[str] = None) -> Set[str]:
        ...
        if prev_line and self.line_start_assign.match(clean_line):
            ...
            if match:
                ...
                if var != '_' and not var.startswith('_'):
                    variables.add(var)
                    self.log(f"    Found cross-line assignment: {var}")
                    return variables
    ...
    def check_function_v13(self, lines: List[Tuple[int, str]], 
                          func_name: str, start_line: int) -> List[Dict]:
        """检查函数（V13版本）
        
        核心改进：
        1. 支持跨行赋值检测（变量名和等号在不同行）
        2. 排除字符字面量（$N等）的误判
        3. 匿名函数创建独立的嵌套作用域
        4. 保持对case/if/receive分支的正确处理
        5. 正确匹配end关键字与对应的结构
        6. 完全移除emoji输出
        """
        issues = []
        ...
        for line_num, line in lines:
            ...
            if func_match and not has_leading_whitespace:
                ...
                if current_func and func_lines:
                    func_issues = self.check_function_v13(func_lines, current_func, func_start)
...
def main():
    ...
    if len(args) < 1:
        print("用法: python check_variable_reassignment_v13.py [选项] <文件或目录路径>")
        print("\n选项:")
        print("  -v, --verbose    显示详细调试信息")
        print("\nV13版本改进:")
        print("  - 支持跨行赋值检测：变量名和等号可以在不同行")
        print("  - 修复字符字面量误判：$N、$A等不再被识别为变量（V12）")
        print("  - 完全移除emoji特殊符号（V11）")
        print("  - 修复匿名函数作用域问题（V10）")
        print("\n示例:")
        print("  python check_variable_reassignment_v13.py src/")
        print("  python check_variable_reassignment_v13.py -v src/test/test_file.erl")
        sys.exit(1)
