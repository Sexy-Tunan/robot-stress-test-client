...
# -*- coding: utf-8 -*-
"""
Erlang 变量重复赋值检测工具 V12
核心改进：
- 修复字符字面量误判：正确识别$N、$A等不是变量赋值
- 完全移除emoji特殊符号（V11）
- 修复匿名函数作用域问题（V10）
- 支持多子句匿名函数（V10）

使用方法:
    python check_variable_reassignment_v12.py src/
    python check_variable_reassignment_v12.py src/test/test_file.erl
"""
...
class VariableAssignmentChecker:
    ...
    def check_function_v12(self, lines: List[Tuple[int, str]], 
        ...
        for line_num, line in lines:
            ...
            if func_match and not has_leading_whitespace:
                ...
                if current_func and func_lines:
                    func_issues = self.check_function_v12(func_lines, current_func, func_start)
...
def main():
    ...
    if len(args) < 1:
        print("用法: python check_variable_reassignment_v12.py [选项] <文件或目录路径>")
        print("\n选项:")
        print("  -v, --verbose    显示详细调试信息")
        print("\nV12版本改进:")
        print("  - 修复字符字面量误判：$N、$A等不再被识别为变量")
        print("  - Erlang变量规则：必须以大写字母或_开头，由字母、数字、_组成")
        print("  - 完全移除emoji特殊符号（V11）")
        print("  - 修复匿名函数作用域问题（V10）")
        print("  - 支持多子句匿名函数（V10）")
        print("\n示例:")
        print("  python check_variable_reassignment_v12.py src/")
        print("  python check_variable_reassignment_v12.py -v src/test/test_file.erl")
        sys.exit(1)
